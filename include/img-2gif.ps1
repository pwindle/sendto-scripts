...
$percent = 0
foreach ($arg in $args) {
	Write-Progress -Activity "Converting to gif" -Status "$percent_formatted% Complete:" -PercentComplete $percent;
	magick mogrify -interlace Plane -colorspace sRGB -format gif $arg
	$count++
